...
unsafe public struct Packet_storage_info {
    ...
    private void Get_more_free_spaces( void* _storage_pointer, int* real_pointer_to_FREE_SPACES ){
        ...
        // [ X, Y, 15, 8, 7, 5 ] => [ X, Y, 5, 7, 8, 15 ]

        // int times_to_flip = ( current_free_space / 2 );
        // for( int index_flip = 0 ; index_flip < times_to_flip ; index_flip++ ){

        //     int index_value_left = ( index_flip );
        //     int index_value_right = ( number_of_free_spaces - index_flip - 1 );

            
        //     int value_left  = real_pointer_to_FREE_SPACES[ index_value_left ];
        //     int value_right = real_pointer_to_FREE_SPACES[ index_value_right ];

        //     real_pointer_to_FREE_SPACES[ index_value_left ] = value_right;
        //     real_pointer_to_FREE_SPACES[ index_value_right ] = value_left;
