...
public static class Directories {
    ...
    public static void Delete_safe( string _path_folder ){

        
        if( _path_folder == null )
            { CONTROLLER__errors.Throw( "tried to delete folder, but the path is null" ); }

        if( !!!( Is_sub_path( _path_folder, Paths_version.path_to_version ) ) )
            { CONTROLLER__errors.Throw( "tried to delete folder, but it is not part of the persistent folder. Path: " + _path_folder ); }

        if( !!!( System.IO.Directory.Exists( _path_folder ) ) )
            { return; }
        
        Directory.Delete( _path_folder, true );
