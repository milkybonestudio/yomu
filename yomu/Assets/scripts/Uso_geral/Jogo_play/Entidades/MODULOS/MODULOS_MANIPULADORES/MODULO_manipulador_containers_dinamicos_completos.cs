...
public class MODULO_manipulador_containers_dinamicos_completos {


        public MODULO_manipulador_containers_dinamicos_completos(  string _nome_gerenciador, string _folder_para_pegar_dados, bool _pode_escrever_no_container, int _numero_inicial_de_slots  ){ 
        ...
        public void Escrever_dados(  int _localizador_id , byte[] _dados ){
                ...
                if( !( pode_escrever_no_container ) )
                        { throw new Exception( "tentou excluir " ); }

                int slot = Pegar_slot( _localizador_id );

                string path = containers_paths[ slot ];
                ...
                FileStream str = new FileStream(  path,  file_mode, file_accees , file_share, _dados.Length , file_options );
